...
なお、ビルド・ストア提出・審査などの「開発・公開の工程」は、この図が表す**実行時の構成とは別次元の話**（CI/CDパイプライン・リリースフローと呼ばれる別の図の領域）のため、ここからは除外した。その内容は[[スマホアプリの開発・テスト・公開の流れ（React Native・Expo）]]、または`docs/requirements.md`に記載している。
